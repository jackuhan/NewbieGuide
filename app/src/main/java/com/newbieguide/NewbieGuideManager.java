                ...
                switch (mType) {
                    case TYPE_LIST:
                        mNewbieGuide.setEveryWhereTouchable(false)
                                .addIndicateImg(R.drawable.left_arrow, ScreenUtils.dpToPx(mActivity, 60), ScreenUtils.dpToPx(mActivity, 110))
                                .addMsgAndKnowTv("这个listview滚动到item6后出现新手引导浮层，\n只有点击我知道啦才会想消失", -ScreenUtils.dpToPx(mActivity, 250))
                                .show();
                        break;
                    case TYPE_COLLECT:
                        mNewbieGuide.addIndicateImg(R.drawable.right, ScreenUtils.dpToPx(mActivity, -70), ScreenUtils.dpToPx(mActivity, 50))
                                .addMsgAndKnowTv("写了一个测试的，随便点击哪里都可以\n废话就不那么多了，你们看看吧", ScreenUtils.dpToPx(mActivity, 150))
                                .show();
                        break;
                }
            }
